...
class ResumeDataService {
    ...
    func fetchProcessedQuestions(completion: @escaping (String) -> Void) {
        ...
        func fetchResponse() async throws -> String {
            guard let url = URL(string: "\(baseURLString)/grade") else {
    ...
    func uploadQuestions(questions: String) async throws {
        guard let url = URL(string: "\(baseURLString)/questions") else {
            throw URLError(.badURL)
        }
        ...
        request.httpMethod = "POST"
        request.setValue("text/plain", forHTTPHeaderField: "Content-Type")
        request.httpBody = questions.data(using: .utf8)
    ...
    func uploadAnswers(answers: String) async throws {
        guard let url = URL(string: "\(baseURLString)/questions") else {
            throw URLError(.badURL)
        }
        ...
        request.httpMethod = "POST"
        request.setValue("text/plain", forHTTPHeaderField: "Content-Type")
        request.httpBody = answers.data(using: .utf8)
